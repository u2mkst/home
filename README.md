...
- 지정 계정(`ufes0603`)으로 로그인 시 일반 학생 메뉴 대신 마스터 페이지 바로가기 제공
...
- **디자인**: 화이트+블루 플랫 디자인 시스템("Toss 스타일"), CSS 커스텀 프로퍼티(`:root` 토큰) 기반 테마링으로 라이트/다크 모드 지원
...
- `master.html`은 이메일 화이트리스트로 접근을 제한하지만, 이는 클라이언트 사이드 체크이며 실제 데이터 보호는 Firebase 보안 규칙에서 이루어져야 합니다.
